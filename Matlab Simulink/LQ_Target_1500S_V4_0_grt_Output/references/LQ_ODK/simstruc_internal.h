 ...
 */
#define _ssSetJacobianPerturbationBounds(S, ptr) \
    (S)->states.statesInfo2->jacPerturbBounds = (ptr)
#define ssGetJacobianPerturbationBounds(S) \
    (S)->states.statesInfo2->jacPerturbBounds
 ...
 */   
#define ssGetJacobianPerturbationBoundsMinVec(S) \
    (S)->states.statesInfo2->jacPerturbBounds->min
 ...
 * 
 */
#define ssGetJacobianPerturbationBoundsMaxVec(S) \
    (S)->states.statesInfo2->jacPerturbBounds->max
...
# define _ssGetOwnerCbd(S) \
          (ssGetParentSS(S) ? \
          ssGetParentSS(S)->work.localMdlInfo->ownerCbd : \
...
# define _ssGetOwnerEbd(S) \
            (ssGetParentSS(S) ? \
            ssGetParentSS(S)->work.localMdlInfo->ownerEbd : \
            (S)->work.localMdlInfo->ownerEbd)

#if !SS_SFCN
# define ssSetOwnerEbd(S,ebd) _ssSetOwnerEbd(S,ebd)
#else
# define ssSetOwnerEbd(S,cbd) \
            ssSetOwnerEbd_cannot_be_used_in_SFunctions
#endif
...
#define _ssGetGlobalRuntimeEventIndex(S, localIndex)                       \
    ((S)->work.localMdlInfo != NULL                                        \
         ...
         : (localIndex)) /*   (uint_T)    */
#if !SS_SFCN
#define ssSetGlobalRuntimeEventIndex(S, localIndex, globalIndex) _ssSetGlobalRuntimeEventIndex(S, localIndex, globalIndex)
#else
...
#define dtaGetGetSizeOfFcn(dta) \
  ((dta)->getSizeOfFcn)
#define dtaSetGetSizeOfFcn(dta, fcn) \
  (dta)->getSizeOfFcn = (fcn)
#define dtaGetSetSizeOfFcn(dta) \
  ((dta)->setSizeOfFcn)
#define dtaSetSetSizeOfFcn(dta, fcn) \
  (dta)->setSizeOfFcn = (fcn)

#define dtaGetGetContainedDataFcn(dta) \
  ((dta)->getContainedDataFcn)
#define dtaSetGetContainedDataFcn(dta, fcn) \
  (dta)->getContainedDataFcn = (fcn)
#define dtaGetSetContainedDataFcn(dta) \
  ((dta)->setContainedDataFcn)
#define dtaSetSetContainedDataFcn(dta, fcn) \
  (dta)->setContainedDataFcn = (fcn)

#define dtaGetGetContainedDataDimsFcn(dta) \
  ((dta)->getContainedDataDimsFcn)
#define dtaSetGetContainedDataDimsFcn(dta, fcn) \
  (dta)->getContainedDataDimsFcn = (fcn)
#define dtaGetSetContainedDataDimsFcn(dta) \
  ((dta)->setContainedDataDimsFcn)
#define dtaSetSetContainedDataDimsFcn(dta, fcn) \
  (dta)->setContainedDataDimsFcn = (fcn)

#define dtaGetGetDataConstructFcn(dta) \
  ((dta)->getDataConstructFcn)
#define dtaSetGetDataConstructFcn(dta, fcn) \
  (dta)->getDataConstructFcn = (fcn)
#define dtaGetSetDataConstructFcn(dta) \
  ((dta)->setDataConstructFcn)
#define dtaSetSetDataConstructFcn(dta, fcn) \
  (dta)->setDataConstructFcn = (fcn)

#define dtaGetGetSerializeFcn(dta) \
  ((dta)->getSerializeFcn)
#define dtaSetGetSerializeFcn(dta, fcn) \
  (dta)->getSerializeFcn = (fcn)
#define dtaGetSetSerializeFcn(dta) \
  ((dta)->setSerializeFcn)
#define dtaSetSetSerializeFcn(dta, fcn) \
  (dta)->setSerializeFcn = (fcn)

#define dtaGetGetDeserializeFcn(dta) \
  ((dta)->getDeserializeFcn)
#define dtaSetGetDeserializeFcn(dta, fcn) \
  (dta)->getDeserializeFcn = (fcn)
#define dtaGetSetDeserializeFcn(dta) \
  ((dta)->setDeserializeFcn)
#define dtaSetSetDeserializeFcn(dta, fcn) \
  (dta)->setDeserializeFcn = (fcn)

#define dtaGetGetSerializeSizeFcn(dta) \
  ((dta)->getSerializeSizeFcn)
#define dtaSetGetSerializeSizeFcn(dta, fcn) \
  (dta)->getSerializeSizeFcn = (fcn)
#define dtaGetSetSerializeSizeFcn(dta) \
  ((dta)->setSerializeSizeFcn)
#define dtaSetSetSerializeSizeFcn(dta, fcn) \
  (dta)->setSerializeSizeFcn = (fcn)
...
#define ssGetSupportedForCodeReuseAcrossModels(S) \
                    ((S)->blkInfo.sfcnFlags.sReuseAcrossModels) 
...
#define ssIsDimensionUnbounded(dim)             \
    ((dim) == (INT_MAX-1))
